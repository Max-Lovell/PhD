...
os.system('cmd /c pscp -pw [password] [email]:/its/home/mel29/metacog/webservice/*.json "C:\\Users\\max_l\\OneDrive - University of Sussex\\3. World Mental States Replication\\Resubmission\\emailer\\webservice"')
...
file_list = glob.glob('C:/Users/max_l/OneDrive - University of Sussex/3. World Mental States Replication/Resubmission/emailer/webservice/*.json')
datal = []
...
dataset.to_csv('C:/Users/max_l/OneDrive - University of Sussex/3. World Mental States Replication/Resubmission/emailer/dataset.csv')
...
for index, row in dataset.iterrows():
    ...
    if last_survey == 'post' or (row['condition'] == 'control' and (elapsed.days < 23 or elapsed.days > 24)): 
        continue #skip if completed post-test (or control and not ready or too old)
    ...
    home_dir = os.path.expanduser('~') # Get the path to the pickle file
    pickle_path = os.path.join(home_dir, 'gmail.pickle')
    creds = pickle.load(open(pickle_path, 'rb'))# Load our pickled credentials
    service = googleapiclient.discovery.build('gmail', 'v1', credentials=creds)# Build the service
    sender_email = "[email]"
    ...
    body = {'raw': raw}
    message1 = body
    message = (service.users().messages().send(userId="me", body=message1).execute()) #comment out this line to run without sending
