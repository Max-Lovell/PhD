...
if (isset($post_data['exp_data']) == true) {//https://www.w3schools.com/php/php_filter.asp
    ...
    $args_san = array(
        ...
        'encoder' => FILTER_SANITIZE_STRING, //note not 'ints' as could have a leading 0
        'distractor' => FILTER_SANITIZE_STRING,
    ...
    $args_val = array(
            ...
            'span' => FILTER_VALIDATE_INT,
            'encoder' => array('filter' => FILTER_VALIDATE_REGEXP, 'options'=>array('regexp'=>'/^\d+$/')),
            'distractor' => array('filter' => FILTER_VALIDATE_REGEXP, 'options'=>array('regexp'=>'/^\d+$/')),
...
if (isset($post_data['file_name']) == true) {
    ...
    $file_name = filter_var($file_name, FILTER_VALIDATE_REGEXP, array('options'=>array('regexp'=>'/^([0-9]{5,6}|[a-z0-9]{24})\_span$/')));//\_(gabor|dots|breath|span)
} else {exit;}
